...
def GoogleSearch(term):
    ...
    writeab = str(query)
    oooooo = open('Data.txt','a')
    oooooo.write(writeab)
    oooooo.close()
    Query = str(term)
    pywhatkit.search(Query)
    os.startfile('DataBase\\ExtraPro\\start.py')
    if 'how to' in Query:
        max_result = 1
...
def Alarm(query):
    ...
    TimeHere.close()
    os.startfile("DataBase\\ExtraPro\\Alarm.py")
def DownloadYouTube():
...
def SpeedTest():
    os.startfile("DataBase\\Gui Programs\\SpeedTestGui.py")
